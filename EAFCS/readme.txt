%%%%%%%%%Algorithm Name: BumpEA-ID (Infeasibility driven evolutionary algorithm embedded with bump hunting)%%%%%%%%%%%%%%%%%%%%
...
4. You are now fully set to run the code. Run the multirun.m script to run the algorithm. Note that, 'parfor' command will run 
   ...
   you don't want to use 'parfor', just replace it with 'for' command and comment 'delete(gcp)' command. That's it. Jusr run it now.
